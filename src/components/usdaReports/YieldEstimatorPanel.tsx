...
const overridesKey = (commodity: string) =>
  `yieldChallenge:overrides:${commodity}`;
...
export default function YieldEstimatorPanel({
  commodity,
  commodityLabel,
  unit = "bu/acre",
}: Props) {
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      .catch(() => {
        if (!cancelled)
          setError(`Could not load ${commodityLabel} yield data.`);
      })
      .finally(() => {
  ...
  useEffect(() => {
    ...
    try {
      ...
      if (Object.keys(userYields).length === 0)
  ...
  const acresSource = useMemo<{
    ...
    data?.priorYearFinal.forEach((r) => {
      if (r.acres != null) priorHarvested.set(r.state, r.acres);
    });

    // 1. Current-year harvested acres ride along on the current yield snapshots
    const currentHarvested = new Map<string, number>();
    data?.currentEstimates.forEach((r) => {
      if (r.acres != null) currentHarvested.set(r.state, r.acres);
    });
    if (data && !data.fellBack && currentHarvested.size > 0) {
      priorHarvested.forEach((v, k) => {
        if (!currentHarvested.has(k)) currentHarvested.set(k, v);
      });
      return {
    ...
    if (
      planting &&
      !planting.fellBack &&
      planting.currentPlantings.length > 0
    ) {
      const planted = new Map<string, number>();
      planting.currentPlantings.forEach((p) => {
        planted.set(p.state, p.acres);
      });
      priorHarvested.forEach((v, k) => {
        if (!planted.has(k)) planted.set(k, v);
      });
      return {
        byState: planted,
  ...
  const baseline = useMemo(() => {
    ...
    data?.currentEstimates.forEach((r) =>
      m.set(r.state.toLowerCase(), r.yieldBu),
    );
    return m;
  }, [data]);
  ...
  async function submitGuess() {
    ...
    try {
      ...
      setSubmitMsg({
        ok: false,
        text: "Submit failed. Try again in a moment.",
      });
    } finally {
      setSubmitting(false);
    }
  ...
  if (loading)
    return (
      <p className={styles.loading}>Loading {commodityLabel} yield data…</p>
    );
  if (error) return <p className={styles.error}>{error}</p>;
  if (!data || stateRows.length === 0) {
    return (
      <p className={styles.empty}>
        No yield data returned. AgriServer may still be fetching from NASS — try
                ...
                return (
                  <tr key={row.state}>
                    ...
                    <td style={{ color: "#777", fontSize: ".82rem" }}>
                      {row.acres
                        ? Math.round(row.acres / 1000).toLocaleString()
                        : "—"}
                    </td>
                  </tr>
                );
                ...
                >
                  ...
                  {guessState && (
                    <>
                      {" "}
                      ·{" "}
                      <strong style={{ color: "#2c4a1e" }}>{guessState}</strong>
                    </>
                  )}
                </div>
                {alreadyGuessed && (
          ...
          <div style={{ maxHeight: "32vh", overflowY: "auto" }}>
            {guesses.length === 0 ? (
              ...
              guesses.map((g, i) => {
                ...
                const badge = !g.updated
                  ? null
                  : g.direction === "up"
                    ? {
                        label: "Raised estimate",
                        fg: "#1a7f37",
                        bg: "#e6f4ea",
                        bd: "#b7e0c2",
                      }
                ...
                return (
                  <div key={g.latestId ?? i}>
                    <div className={styles.guessCard}>
                      ...
                      >
                        <div
                          className={styles.guessValue}
                          style={{ margin: 0 }}
                        >
                          {g.estimate.toFixed(1)}
                        </div>
                        {g.direction &&
                          g.direction !== "same" &&
                          g.delta != null && (
                    ...
                    {open && (
                      <div
                        style={{
                          ...
                          padding: ".4rem 1rem .65rem 1.5rem",
                          fontFamily: "Lato, sans-serif",
                        }}
                      >
                        {log === null || log === undefined ? (
                          ...
                          [...log].reverse().map((rev, j) => {
                            ...
                            const d = earlier
                              ? +(rev.estimate - earlier.estimate).toFixed(1)
                              : null;
                            return (
                              <div
                                key={rev.id ?? j}
                              ...
                              >
                                ...
                                <strong style={{ color: "#2c4a1e" }}>
                                  {rev.estimate.toFixed(1)}
                                </strong>
                                {d != null && d !== 0 && (
                                  <span
                                    className={
                                      d > 0 ? styles.diffUp : styles.diffDown
                                    }
                                    style={{ minWidth: 0 }}
                                  >
                                    {d > 0 ? "+" : ""}
