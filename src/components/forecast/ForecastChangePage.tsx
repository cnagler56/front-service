...
export default function ForecastChangePage() {
  const { locations, loading, error, save, remove, refreshAll } = useForecastLocations();
  const { user } = useUser();
  ...
  async function handleRefreshAll() {
    ...
    try {
      ...
      alert('Refresh failed — check that you are signed in as an admin and the server is running.');
    } finally {
      setRefreshing(false);
    }
  }
  ...
  const states = useMemo(() => {
    ...
    for (const l of locations) { const st = parseState(l.name); if (st) s.add(st); }
      ...
      <div className={styles.section}>
        ...
        <div className={styles.sectionBody}>
          ...
          {!loading && locations.length > 0 && (
            <div style={{
              ...
              <select
                ...
                style={{
                  padding: '.45rem .6rem', border: '1px solid #cdd6bd', borderRadius: 4,
                  fontSize: '.85rem', fontFamily: 'Lato, sans-serif', color: '#1a2e0f',
          ...
          {!loading && locations.length > 0 && filtered.length === 0 ? (
            ...
            <LocationsGrid
              locations={filtered}
              onEdit={isAdmin ? loc => setModal({ open: true, editing: loc }) : undefined}
